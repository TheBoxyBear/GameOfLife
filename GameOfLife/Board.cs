    ...
    public class Board
    {
        ...
        private int LivingNeighbours(int x, int y)
        {
            ...
            if (x > 0)
            {
                ...
                if ((north = y > 0) && Cells[x - 1, y - 1])
                    aliveCount++;
                // There is a cell on the bottom left
                if ((south = y > YLimit) && Cells[x - 1, y + 1])
                    aliveCount++;
            }
            ...
            if (x < XLimit)
            {
                ...
                if ((north || (north = y > 0)) && Cells[x + 1, y - 1])
                    aliveCount++;
                // There is a cell on the bottom right
                if ((south || (south = y < YLimit)) && Cells[x + 1, y + 1])
                    aliveCount++;
            }

            // There is a cell on the top
            if ((north || y > 0) && Cells[x, y - 1])
                aliveCount++;

            // There is a cell on the bottom
            if ((south || y < YLimit) && Cells[x, y + 1])
                aliveCount++;
